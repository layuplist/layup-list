...
from web.models import Vote, Course
...
class VoteTestCase(TestCase):
    ...
    def test_vote(self):
        gv = factories.VoteFactory(category=Vote.CATEGORIES.GOOD)
        lv = factories.VoteFactory(
            course=gv.course,
            user=gv.user,
            category=Vote.CATEGORIES.LAYUP
        )
        c = gv.course
        u = gv.user
        ...
        self.assertEqual((None, False,), Vote.objects.vote(
            5, c.id, Vote.CATEGORIES.LAYUP, u))

        self.assertEqual(gv.value, 0)
        self.assertEqual(lv.value, 0)
        self.assertEqual(c.layup_score, 0)
