...
from utils import clean_text_to_raw_words
...
for course in Course.objects.exclude(description=None).exclude(description=""):
    ...
    word_jumble = [clean_text_to_raw_words(course.description)]
    word_jumble.append(clean_text_to_raw_words(course.title))
    # reviews are very noisy -- they tend to be similar between classes
    if INCLUDE_REVIEWS:
        for review in course.review_set.all():
            word_jumble.append(clean_text_to_raw_words(review.comments))
...
# zero out crosslistings and same titles, so only one rep for each crosslisting
covered_ids = set()
